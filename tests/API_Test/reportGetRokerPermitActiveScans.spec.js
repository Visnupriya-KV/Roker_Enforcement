...
test('API_ReportGetRokerPermitActiveScans_Test: Capture cookies and call Get Roker Permit Active Scans API', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.getRokerPermitActiveScansApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: reportGetRokerPermitActiveScansRequestBody, // Pass the request body from JSON
  });
