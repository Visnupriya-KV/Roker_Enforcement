...
import violationSearchExportRequestBody from '../API_JSON/violationSearchExportRequestBody.json';
...
test('API_ViolationSearchExport_Test: Capture cookies, call Violation Search Export API, and save XML file', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.violationSearchExportApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: violationSearchExportRequestBody, // Pass the request body from JSON
  });
  ...
  const outputFilePath = path.join(exportsDir, 'violationSearchExport.xml'); // Define the output file path
