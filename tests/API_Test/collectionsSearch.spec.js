...
test('API_CollectionsSearch_Test: Capture cookies and call Collections Search API', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.collectionsSearchApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: collectionsSearchRequestBody, // Pass the request body from JSON
  });
