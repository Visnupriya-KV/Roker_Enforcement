...
import searchRequestBody from '../API_JSON/searchRequestBody.json';
...
test('API_Search_Test: Capture cookies and call Violation Search API', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.violationSearchApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: searchRequestBody, // Pass the request body from JSON
  });
