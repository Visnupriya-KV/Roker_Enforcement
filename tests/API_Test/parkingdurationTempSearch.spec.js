...
test('API_ParkingDurationTempSearch_Test: Capture cookies and call Parking Duration Temp Search API', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.parkingDurationTempSearchApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: parkingDurationTempSearchRequestBody, // Pass the request body from JSON
  });
