...
test('API_PlateActivitySearchExport_Test: Capture cookies, call Plate Activity Search Export API, and save file', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.plateActivitySearchExportApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: plateActivitySearchExportRequestBody, // Pass the request body from JSON
  });
  ...
  // Step 7: Save response as a file
  ...
  const outputFilePath = path.join(exportsDir, 'plateActivitySearchExport.xml'); // Define the output file path
  fs.writeFileSync(outputFilePath, responseBody, 'utf8'); // Write the response to the file
