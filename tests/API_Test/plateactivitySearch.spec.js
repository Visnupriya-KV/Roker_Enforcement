...
import plateActivitySearchRequestBody from '../API_JSON/plateActivitySearchRequestBody.json';
...
test('API_PlateActivitySearch_Test: Capture cookies and call Plate Activity Search API', async ({ page }) => {
  ...
  const apiUrl = apiEndpoints.plateActivitySearchApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: plateActivitySearchRequestBody, // Pass the request body from JSON
  });
