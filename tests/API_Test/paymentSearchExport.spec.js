...
import paymentSearchExportRequestBody from '../../data/API_JSON/paymentSearchExportRequestBody.json';

test('API_PaymentSearchExport_Test: Capture cookies, call Payment Search Export API, and save XML file', async ({ page }) => {
  let cookies = '';
  ...
  const apiUrl = apiEndpoints.paymentSearchExportApi; // Ensure this is defined in apiEndpoints.json
  console.log('Making API call to:', apiUrl);

  const response = await apiContext.post(apiUrl, {
    data: paymentSearchExportRequestBody, // Pass the request body from JSON
  });
  ...
  const outputFilePath = path.join(exportsDir, 'paymentSearchExport.xml'); // Define the output file path
